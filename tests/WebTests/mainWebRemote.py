...
def safe_create(label, module):
    try:
        comp = secure_runtime.remoteCreate(REMOTE_RUNTIME, "web", module, label, False)
...
def safe_connect(src_id, tgt_id, intf):
    try:
        if secure_runtime.remoteConnect(REMOTE_RUNTIME, "web", src_id, tgt_id, intf):
...
def safe_disconnect(src_id, tgt_id, intf):
    try:
        if secure_runtime.remoteDisconnect(REMOTE_RUNTIME, "web", src_id, tgt_id, intf):
