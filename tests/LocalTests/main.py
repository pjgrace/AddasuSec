...
def run_tests():
    try:
        ...
        print(f"Caclulator1(8 - 2) = {calc1.sub(8, 2)}")
...
try:
    calc1 = addasuSec.create("plain", "Examples.Calculator", "Calculator1", False)
    ...
    #test a delete
    deleteAdd = addasuSec.delete("plain", "Adder1")
    print(f"Component delete: Adder1 is removed is {deleteAdd}")
    
    #test recreate the deleted component
    add1 = addasuSec.create("plain", "Examples.Adder", "Adder1", False)
    print(f"Component created: component identifier is {addasuMeta.getLabel(add1)}")
    
    sub1 = addasuSec.create("plain", "Examples.Subber", "Subber1", False)
...
# 1) to create another component with the same name, this should raise and exception and continue
# 2) delete a component that doesn't exist
# 3) create a component with an invalid name
try:
    deleteAdd = addasuSec.delete("plain", "Adder5")
    print(f"Component delete: Adder5 is removed is {deleteAdd}")
except ComponentException as e:
    print("Exception deleting a component:", e)

try:
    newadd1 = addasuSec.create("plain", "Examples.Adder", "Adder1", False)
except ComponentException as e:
    print("Exception creating a component:", e)

try: 
    newadd2 = addasuSec.create("plain", "Examples.Adder", "Adder 1", False)
except ComponentException as e:
    print("Exception creating a component:", e) 

# Connect all the components and then run the calls
full_connection()
run_tests()
...
try:
    ...
    if addasuSec.disconnect("plain", calc2,add1,"Examples.IAdd"):
        print(f"Successfully connected {addasuMeta.getLabel(calc2)} to {addasuMeta.getLabel(add1)}")
        try:
    ...
    if addasuSec.disconnect("plain", calc2,sub1,"Examples.ISub"):
        print(f"Successfully connected {addasuMeta.getLabel(calc2)} to {addasuMeta.getLabel(sub1)}")
        try:
...
addasuMeta.setInterfaceAttributeValue("Calculator1", "Examples.IAdd",  "peter", "bob")
if addasuMeta.getInterfaceAttributeValue("Calculator1", "Examples.IAdd",  "peter") == "bob":      
    print("Calc1.IAdd meta data set correctly")
else:
    print("Calc1.IAdd meta data set incorrectly")

addasuMeta.setInterfaceAttributeValue("Calculator2", "Examples.IAdd",  "Variation", 77)
if addasuMeta.getInterfaceAttributeValue("Calculator2", "Examples.IAdd",  "Variation") == 77:        
    print("Calc2.IAdd meta data set correctly")
else:
    print("Calc2.IAdd meta data set incorrectly")
    
addasuMeta.setInterfaceAttributeValue("Calculator1", "Examples.ISub",  "Variation", 98)
if addasuMeta.getInterfaceAttributeValue("Calculator1", "Examples.ISub",  "Variation") == 98:       
    print("Calc1.ISub meta data set correctly")
else:
    print("Calc1.ISub meta data set incorrectly")
