...
from Runtimes.runtime import runtime, ComponentException, ComponentException
...
def full_connection():
    ...
    assert addasuSec.connect(WEB_SERVER, calc1, add1, IADD)
    assert addasuSec.connect(WEB_SERVER, calc2, add1, IADD)
    assert addasuSec.connect(WEB_SERVER, calc1, sub1, ISUB)
    assert addasuSec.connect(WEB_SERVER, calc2, sub1, ISUB)
...
try:
    calc1 = addasuSec.create(WEB_SERVER, CALC_CLASS, CALC1_NAME, False)
    calc2 = addasuSec.create(WEB_SERVER, CALC_CLASS, CALC2_NAME, False)
...
def test_disconnect_and_failures():
    ...
    for c, t, iface, url in [
        ...
        assert addasuSec.disconnect(WEB_SERVER, c, t, iface)
        try:
